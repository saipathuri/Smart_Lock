...
scan ()
{
	...
	then
		...
		# Create file to store MAC addresses
		if [ -f $macAddrFile ]
		then
			rm -f $macAddrFile
		fi
		...
		# Output all device information on local network
		...
		echo "$(cat $macAddrFile | wc -l) devices have been found."
